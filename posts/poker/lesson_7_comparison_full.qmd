...
\end{cases} \tag{8}\label{eq:z1z2-equations}
$$
...
\end{cases}
}_{\text{Part 2: } z_1^*, x_2^*, z_2^* \text{ Dependent Terms}}  \tag{9}\label{eq:Gx-pieces}
\end{align}
$$
...
$$
\int_0^1 \int_0^1 J(x,y) \, dy \, dx =  \frac{P}{2}(1-z_1^*)^2 + B(1-z_2^*)^2  \tag{10}\label{eq:Jxy-integral-with-constraints}
$$
$$
\begin{align}
G_1(x) &= \begin{cases} 
(P+B)y_1^* - B & \text{, if } x \le y_1^* \\
(P+2B)x - B(1+y_1^*) & \text{, if } x > y_1^*
\end{cases}  \tag{11}\label{eq:G1x} \\
G_2(x) &=
\begin{cases} 
0 & \text{, if } x < z_1^* \\
P(x - z_1^*) & \text{, if } z_1^* \le x \le z_2^* \\
(P+2B)(x+z_1^*) - 2B & \text{, if } x \ge z_2^*
\end{cases}  \tag{12}\label{eq:G2x} \\
G(x) &= G_1(x) - G_2(x) \\
\end{align}
$$
我們把 $G$ 拆分成兩個，第二個加了負號，這樣一來斜率都是正的了。還有做了一些簡單的代數代換，讀者可自行驗證。

![Plot of G1 and G2](images/lesson_7/G1_G2.png){#fig-plot-G1-G2}

單純看這個圖，我們就知道如果 $a_1(x)$ 要最大化 Lagrangian，那就是極化的，在兩端 $G(x)$ 是正的部分全 1，中間負的部分全 0。

但真的是這樣嗎?

因為 $G_1(x), G_2(x)$ 在開頭和結尾的斜率都一致，所以會不會 **剛好重和**。那樣一來 $G(x)$ 就是零，所以 $a_1(x)$ 可以是任意值。無差異! Indifference!

但也不一定要取到剛好一樣，如果可以的話，只要讓 $G(x)$ 一定小於等於 0 就行了。Player B 有辦法辦到嗎?
我們解端點 0 和 1 時的不等式(這邊的 $G_2(x)$ 我使用沒有帶面積條件的版本 \eqref{eq:Gx-pieces}):
$$
\begin{cases}
(P+B)y_1^* - B \leq 0 \\
(P+2B) - B(1+y_1^*) \leq \Big( (P+2B) - B(1 - z_1^* + z_2^*) \Big) \\
\end{cases}
$$
也就是
$$
\begin{cases}
\displaystyle y_1^* \leq \frac{B}{P+B} \\
\displaystyle z_2^* - z_1^* \leq y_1^* \\
\end{cases} \tag{13}\label{eq:inequalities-from-endpoints}
$$
假設可以取到，那麼 $G(x) \leq 0$ 對所有 $x \in [0,1]$ 都成立，這樣 Lagrangian 最大值就是 $J(x,y)$ 的積分值 \eqref{eq:Jxy-integral-with-constraints}。還是帶有 $z_1^*, z_2^*$，所以我們要把它具體解出來。

BTW, 如果 $G$ 讓出一點點正的部分給 $a_1(x)$ 可以因此壓低的 $J$ 的積分值的話，那我這樣就解錯了，但我們猜應該不會，這是這類 Indifference principle 不嚴謹的地方。而且根據該原則，其實上面兩個等號一定會成立，因為那是**最**讓 Player A 感受到無差異的條件: 所有 $G(x)=0$ 的地方 $a_1(x)$ 的取值都不影響結果。如果嚴格小於，那 Player A 在那裡的 $a_1(x)$ 一定會取 0。而直覺上 Player B 會在另個其他某個地方付出代價。這部分也不嚴謹。

還是來算算看，根據面積條件 \eqref{eq:z1z2-equations}，假設 $z_1^* = Bt$，那麼 $z_2^* = 1 - (P+B)t$，所以 $z_2^* - z_1^* = 1 - (P+2B)t$，而他最多是 $B/(P+B)$，所以
$$
t \geq \frac{P}{(P+B)(P+2B)}
$$
...
\begin{align}
\mathcal{L}_{max} 
&= \frac{P}{2}(1 - z_1^*)^2 + B(1 - z_2^*)^2 \\
&= \frac{P}{2}(1 - Bt)^2 + B\Big((P+B)t\Big)^2  \\
&= \frac{P}{2} - PBt + \Big( B(P+B)^2 + \frac{PB^2}{2} \Big) t^2 \\
\end{align}
$$
是個開口向上的二次函數，因為 $t$ 是 Player B 控制的，要選 $t$ 讓它最小化。而其對稱軸位於
$$
t_{\min} = \frac{PB}{2\Big( B(P+B)^2 + \frac{PB^2}{2} \Big)} = \frac{P}{(2P+B)(P+2B)} < \frac{P}{(P+B)(P+2B)} \leq t
$$
所以最佳的 $t$ 果然是取到等號成立:
$$
t^* = \frac{P}{(P+B)(P+2B)}  
...
$$
\begin{align}
&z_1^* = \frac{BP}{(P+B)(P+2B)} , \quad z_2^* = \frac{2B}{P+2B} \\
&y_1^* = \frac{B}{P+B} \\
\end{align}  \tag{14}\label{eq:final-z1z2y1}
$$

以上推導應該還要再驗證，若不等式 \eqref{eq:inequalities-from-endpoints} 不成立，$a_1(x)\cdot G(x)$ 所增加的量，會比 $J(x,y)$ 所減少的量還多。這樣才保證 Player B 成功使 Lagrangian 最小化。但其實不太可能，因為如此一來，Player A 就不是針對 Player B 打出最佳攻擊了。但在 Nash Equilibrium 時，雙方都是最佳防守狀態，也都是彼此的最佳攻擊。所以在 Nash Equilibrium 狀態下，以上推導是成立的。

而 Player B 取這個 $t^*$ 時，Lagrangian 是多少呢? 因為上面 $\mathcal{L}_{max}$ 的式子中，$t=0$ 時會是 $P/2$ 也就是池底的一半，所以在 $[0, 2t_{\min}]$ 之間 $\mathcal{L}_{max}$ 會小於 $P/2$，超過 $2t_{\min}$ 後就會大於 $P/2$。而我們的 $t^*$ 是小於 $2t_{\min}$ 的!
$$
t^* = \frac{P}{(P+B)(P+2B)} < 2t_{\min} = \frac{2P}{(2P+B)(P+2B)}
$$
所以 Player A 的整體期望值是小於池底一半的! 也就是**先手劣勢**。

如此一來 Game Value 有了，具體來說是:
$$
\begin{align}
&\max_{a_1, a_2} \min_{b_1, b_2} \mathcal{L}(a_1, a_2, b_1, b_2)    \\
&\quad = \frac{P}{2} - t^* B \left(P - \frac{(2P+B)(P+2B)}{2} t^* \right)    \\
&\quad = \frac{P}{2} - \frac{P}{(P+B)(P+2B)}\cdot B \left( P - \frac{P(2P+B)}{2(P+B)} \right) \\
&\quad = \frac{P}{2} - \frac{P^2B^2}{2(P+2B)(P+B)^2}  \tag{15}\label{eq:game-value-final}
\end{align}
$$
總結以上的解法，Player B 打出最佳防守 \eqref{eq:final-z1z2y1} 時，是不需要知道 Player A 的策略的，所以這並非 Player B 的最佳攻擊策略。Player A 此時被無差異化了，所以 $a_1(x)$ 在 $[z_1^*, z_2^*]$ 是 $0$，其他地方可以任意。

但若 Player B 可以根據 Player A 的策略來動態調整，那麼 Player B 的 $y_1^*, z_1^*, z_2^*$ 就會依照 \eqref{eq:z1z2-equations} 的面積比例去切分。此時 Player A 要如何選擇 $a_1(x)$ 才能最大化 Lagrangian? 

如果繼續沿用剛才的邏輯，當 Nash Equilibrium 時，Player B 的最佳防守策略 \eqref{eq:final-z1z2y1} 會是對 Player A 的最佳攻擊，所以其實 \eqref{eq:final-z1z2y1} 也剛好會把 $a_1(x)$ 依照 \eqref{eq:z1z2-equations} 的比例去切分。

所以說，$(1-a_1(x))$ 在 $[z_1^*, z_2^*]$ 部分的面積，比上 $[z_2^*, 1]$ 部分的面積，會是 $(B + \frac{P}{2}) : \frac{P}{2}$ 的比例。而因為 $a_1(x)$ 在 $[z_1^*, z_2^*]$ 是 $0$，所以 $(1-a_1(x))$ 在這區間的面積就是 $z_2^* - z_1^*$。所以剩下要算的就是 $a_1(x)$ 在 $[z_2^*, 1]$ 的部分是多少。
$$
\frac{1 - z_2^* - A_{value}}{z_2^* - z_1^*} = \frac{P}{2B + P} 
$$
帶 \eqref{eq:final-z1z2y1} 解出
$$
A_{value} = \frac{P}{P + 2B}\frac{P}{P+B}
$$
而 $a_1(x)$ 在 $[0, z_1^*]$ 的部分面積需要是 $A$ 的 $\frac{B}{P+B}$，
所以
$$
A_{bluff} = \frac{B}{P+B} A_{value} = \frac{P^2B}{(P+2B)(P+B)^2}
$$

![Final $a_1(x)$ Optimal Strategy](images/lesson_7/a1_optimal.png){#fig-final-a1-optimal-strategy}

現在算出了 $a_1(x)$ 在兩邊的面積是多少，那形狀呢? 剛才的 Lagrangian 似乎就被面積決定了，也沒有提到形狀的問題。沒錯，Lagrangian 只跟面積有關，所以形狀可以任意。
